...
def test_loading_pickle():
    ...
    assert inputs.size() == torch.Size(
        (batch_size, mel_channels, sequence_len)
    ), "Pickle Training Dataset Images dimension mismatches"
    assert (
        len(targets) == batch_size
    ), "Pickle Training Dataset labels dimension mismatches"
    assert isinstance(targets[0].item(), int), "Incorrect Target Data Types"
    assert (
        min(targets) >= 0 and max(targets) <= len(config.LABELS) - 1
    ), "Incorrect Target Values"

    inputs, targets = next(iter(val_loader))
    assert inputs.size() == torch.Size(
        (batch_size, mel_channels, sequence_len)
    ), "Pickle Validation Dataset Images dimension mismatches"
    assert (
        len(targets) == batch_size
    ), "Pickle Validation Dataset labels dimension mismatches"
    assert isinstance(targets[0].item(), int), "Incorrect Target Data Types"
    assert (
        min(targets) >= 0 and max(targets) <= len(config.LABELS) - 1
    ), "Incorrect Target Values"
    ...
    assert inputs.size() == torch.Size(
        (batch_size, mel_channels, sequence_len)
    ), "Pickle Testing Dataset Images dimension mismatches"
    ...
    assert inputs.size() == torch.Size(
        (batch_size, mel_channels, sequence_len)
    ), "Pickle Prediction Dataset Images dimension mismatches"


# def test_loading_wav():
#     """Test for loading wav files
#     """
#     batch_size = 3
#     padding_length = 20000
#     fft_window, fft_overlap = 512, 128
